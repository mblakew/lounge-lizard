...
class MsTeamsLogin {
  ...
  _bruh(success, result) {
    if (!success) return
    if (typeof result === "string" && /^https:\/\/teams.microsoft.com\/go#id_token=/.test(result)) {
      console.log("got the id_token! " + result.split("https://teams.microsoft.com/go#id_token=")[1])
      this.browser.stop()
      this.window.close()
    }
  }
