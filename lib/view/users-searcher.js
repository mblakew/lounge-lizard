...
// edit this to be users
  ...
  async onClick(view, event) {
    ...
    if (member) {
      if (this.account.findChannelByUserId(member.id)) {
        let direct_message = await this.account.joinDM(member.id)
        let channel = direct_message.channel.id  
        this.mainWindow.channelsPanel.selectChannelById(channel)
      } else if (this.account.canJoinDm) {
        /***
         * The code in this block remains tightly coupled to the Slack client (primarily
         * because, in the case of the Slack client, joinDM is returning a promise that
        ...
        let direct_message = this.account.joinDM(member.id)
  ...
  onTextChange(view) {
    ...
    this.applyFilter((c) => String(c.name).includes(text) || String(c.realName).includes(text))
  }
}
