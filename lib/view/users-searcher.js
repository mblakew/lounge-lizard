...
// edit this to be users
  ...
  draw(view, painter, dirty) {
    ...
    for (let i = start; i <= end; ++i) {
      ...
      // is user private?
      const prefix = member.isPrivate ? 'θ' : '@'
      painter.drawText(prefix + member.name, bounds, {font: NAME_FONT})
      // Description.
      bounds.y += 20
      // trying to get the real name
      painter.drawText(String(member.real_name), bounds, {font: DESP_FONT})
      // Seperator.
  ...
  onTextChange(view) {
    ...
    this.applyFilter((c) => String(c.name).includes(text) || String(c.real_name).includes(text))
  }
}
