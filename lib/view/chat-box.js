...
const messageHtml = fs
  .readFileSync(path.join(__dirname, "chat", "message.html"))
  .toString();
const reactHtml = fs
...
const pageTemplate = handlebars.compile(
  fs.readFileSync(path.join(__dirname, "chat", "page.html")).toString()
);
...
class ChatBox {
  constructor(mainWindow) {
    ...
    this.browser.addBinding(
      "openLinkContextMenu",
      this._openLinkContextMenu.bind(this)
    );
    ...
    this.browser.addBinding(
      "setMessageReaction",
      this._setMessageReaction.bind(this)
    );
    ...
    this.browser.addBinding(
      "notifyDisplaying",
      this._notifyDisplaying.bind(this)
    );
    this.browser.addBinding(
      "notifyNotDisplaying",
      this._notifyNotDisplaying.bind(this)
    );
    ...
    const gifPath = fs.realpathSync(
      path.join(__dirname, "chat", "loading@2x.gif")
    );
    ...
    this.loadingIndicatorWrapper.setBackgroundColor(
      theme.chatBox.backgroundColor
    );
  ...
  async loadChannel(messageList) {
    ...
    this.subscription = {
      ...
      onDeleteMessage: messageList.onDeleteMessage.add(
        this._deleteMessage.bind(this)
      ),
      onModifyMessage: messageList.onModifyMessage.add(
        this._modifyMessage.bind(this)
      )
    };
    ...
    if (messageList === this.messageList) {
      ...
      if (process.env.WEY_DEBUG === "1") fs.writeFileSync("page.html", html);
      this.loadedTimes++;
  ...
  _newMessage(message) {
    if (this.threadId && this.threadId !== message.threadId) return;
    ...
    const fromCurrentUser =
      message.user.id === this.messageList.account.currentUserId;
    this.browser.executeJavaScript(
      `window.addMessage(${JSON.stringify(
        html
      )}, ${firstUnread}, ${fromCurrentUser})`,
      () => {}
    );
  }
  ...
  _openThread(id) {
    const win = windowManager.windows.find(
      w => w.chatBox && w.chatBox.messageList.id === id
    );
    if (win) {
      win.window.activate();
    } else {
  ...
  async _fetch(u, callbackId) {
    ...
    if (this.messageList.account.authorizationHeader)
      options.headers = {
        Authorization: this.messageList.account.authorizationHeader
      };
    // Fetch the URL and disgard result if page hasx been changed.
    ...
    // Report result back.
    this.browser.executeJavaScript(
      `window.executeCallback(${callbackId}, "${result}")`,
      () => {}
    );
  }
...
handlebars.registerHelper("isFirstUnread", function(
  messageList,
  message,
  options
) {
  if (messageList.firstUnreadTs === message.timestamp) return options.fn(this);
  else return options.inverse(this);
});

handlebars.registerHelper("isChannel", function(messageList, options) {
  if (messageList.type === "channel") return options.fn(this);
  else return options.inverse(this);
});

handlebars.registerHelper("canStartThread", function(
  messageList,
  message,
  options
) {
  if (
    messageList.type === "channel" &&
    (!message.threadId || message.isThreadParent)
  )
...
const fontStyle = fs
  .readFileSync(path.join(__dirname, "chat", "font.css"))
  .toString();
