...
class ChatBox {
  ...
  _newMessage(message) {
    ...
    if (message.timestamp === this.messageList.firstUnreadTs) firstUnread = true
  ...
  async _getProfile(id, timestamp) {
    ...
    if (win) {
      ...
      if (this.messageList.type === 'channel' || this.messageList.type === 'dm') {
        ...
        if (!dm) {
          dm = this.mainWindow.chatBox.messageList.account.joinDM(profile.user.id)
...
handlebars.registerHelper('canStartThread', function(messageList, message, options) {
  if (messageList.type === 'channel' && (!message.threadId || message.isThreadParent)) return options.fn(this)
  else return options.inverse(this)
})
