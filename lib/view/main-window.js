    ...
    if (this.channelsPanel.selectedChannelItem ||
      ...
      for (let channel of channels){
       ...
       for(let parent of threadParents){
         ...
         thread.channelName = channel.name; //save channel name to thread
         
         //TODO: get user id of current user, check against userids of messages
         //only push threads that contain user id
     ...
     // TODO:
      //load messages from all channels >> determine threads
      //make messagelist from thread messages
      //load messagelist into chatbox

      this.channelsSearcher.unload()
      this.chatBox.view.setVisible(true);
      this.chatBox.loadThreads(threads);
      require('../controller/config-store').serialize();
  }

  
  containsMessagesFromUser(messages, userId){
