...
class MessageList {
  ...
  async dispatchMessage(message) {
    ...
    this.onMessage.dispatch(message, this)
  }
  ...
  async listEmotes(id, timestamp, name, reacted) {
    throw new Error("Should be implemented by subclasses");
  }
  ...
  showReactPicker(id, timestamp, emojiList) {
    const message = this.findMessage(id);

    if (message) {
      message.emojiList = emojiList;
      this.onModifyMessage.dispatch(id, message);
    }
