...
class TeamsAccount extends Account {
  ...
  async updateChannels() {
    ...
    const helper = new TeamsApiHelper();

    // TODO: Get the correct team id based on what team the user has logged in to
    const teamsChannels = await helper.getChannels("be79082b-c2b2-429a-9aba-47c147335c55");

    // Microsoft's "get channels" API returns less data than Slack does.  For now, just
    // hard-coding initial values for some of the properties that are returned by Slack
    // but not included in Microsoft's API response.
    // The better approach here is probably to just pass the original, untransformed data
    // in to the TeamChannels constructor and let it decide how to interpret it.
    // There is precedent for this in the SlackChannels class.  This approach would also 
    // help if we end up making multiple API calls to get all of the data we need to 
    // create a TeamsChannel object and would help avoid any potential code related to 
    // unused properties (either from the Slack API responses or the Teams API responses).
    const transformedChannels = teamsChannels
      .map(c => {
        return {
          id: c.id,
          name: c.displayName,
          ...
          is_member: true,
          has_unreads: false,
          last_read: '1587627752.002100',
          mention_count_display: 0
        };
      });

    this.channels = transformedChannels.concat(groups)
