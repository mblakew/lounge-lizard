...
class TeamsAccount extends Account {
  ...
  setupTeamsClient(token) {
    ...
    /***
     * TODO: Determine which of the events listed below can be supported with MS Teams
     *       and implement
     */
  ...
  setupRTMClient(token) {
    ...
    require('./private-apis').extend(this.rtm)
    // this.rtm.once('authenticated', this.slackReady.bind(this))
    this.rtm.start({ batch_presence_aware: true })
  }
  ...
  async ready() {
    ...
    // TODO: Need to determine if there is a way to get the team icon from
    ...
    // TODO: Need to determine if we need this URL and, if so, how to construct it.
    ...
    // Current user (currentUserName is unused in MS Teams implementation)
    this.currentUserId = (await this.apiHelper.getCurrentUser()).id;
  ...
  async updateEmoji() {
    /***
     * TODO: Determine if there is a way to get an emoji list with the MS Graph API
  ...
  updatePresenceSubscription() {
    /***
     * TODO: Determine how we can tell (with the MS Graph API) what a user's status 
  ...
  // async joinDM(us) {
  //   // TODO: Figure out how this actually works so we can replicate the functionality for MS Teams
  ...
  serialize() {
    /***
     * TODO:
     * Change this last because once we change this, we should expect that none of our
     * other classes will still be able to use the Slack client
     */
