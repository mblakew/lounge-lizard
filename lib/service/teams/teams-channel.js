...
class TeamsChannel extends Channel {
  constructor(account, event) {
    ...
    this.emojiConverter = {};
    require("./emoji")(this.emojiConverter);
  }
  ...
  async readMessagesImpl() {
    ...
    for (const m of smsgs) { // need to get additional info.
      ...
      // TODO: extract emoji conversion here
      let text = m.text;
      const emojiConverter = this.emojiConverter;
      const asciiRegexp = emojiConverter.asciiRegexp;
      const asciiList = emojiConverter.asciiList;
      const jsEscapeMap = emojiConverter.jsEscapeMap;
      text = text.replace(new RegExp(asciiRegexp), function (m) {
        return emojiConverter.objectFlip(jsEscapeMap)[asciiList[m]];
      });
      text = this.emojiConverter.toImage(text);

      // END EMOJI CONVERSION
      m.text = text;
    }
    return smsgs
  }
