...
const apiPaths = {
  ...
  getJoinedTeams: "/v1.0/me/joinedTeams",
 ...
 */
const loginPaths = {
  deviceAuthorizationRequest: "/common/oauth2/v2.0/devicecode",
  retrieveAccessToken: "/common/oauth2/v2.0/token",
}
...
class TeamsApiHelper {
  ...
  getScope() {
    return scope.map(permission => `https://graph.microsoft.com/${permission}`).join(' ')
  }

  async retrieveAccessToken() {
    const options = {
      baseURL: loginHost,
      url: loginPaths.retrieveAccessToken,
      method: 'post',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      },
  ...
  async getJoinedTeams() {
    const response = await this.invokeTeamsApiGetCollection(apiPaths.getJoinedTeams)
    const teams = []

    response.reduce((accumulator, { id, displayName }) => {
      accumulator.push({ id: id, displayName: displayName })
    }, teams)
  ...
  async refreshToken() {
    const { refresh_token, expires_in, access_token } = await this.retrieveAccessToken()
    this.accessToken = access_token;
    this.code = refresh_token
    // Get "expires_in" from response, subtract 5 minutes, and convert to milliseconds
    const expireTokenTimeout = (expires_in - 300) * 1000
