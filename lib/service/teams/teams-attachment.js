...
const {teamsMarkdownToHtmlSync} = require('./message-parser')

class TeamsAttachment extends Attachment {
  constructor(account, att) {
    super(att.text ? teamsMarkdownToHtmlSync(account, att.text) : null)
    if (att.pretext)
      this.preText = teamsMarkdownToHtmlSync(account, att.pretext)
    if (att.color)
...
module.exports = TeamsAttachment
